...
typedef std::vector<char> Packed2;
...
public:
  // outbound messages
  inline Msg(csv type, size_t max_size = DOC_DEFAULT_MAX_SIZE)
      : type(type.data(), type.size()), doc(max_size), packed(max_size / 2) {
    doc[TYPE] = type;
    doc[NOW_US] = rut::now_epoch<Micros>().count();
  }

  // inbound messages
  inline Msg(size_t max_size = DOC_DEFAULT_MAX_SIZE)
      : type(io::READ_MSG), doc(max_size), packed(max_size / 2) {}
  ...
  string type;
  DynaDoc doc;
  Packed2 packed; // both RX and TX need at least MSG_LEN_SIZE
